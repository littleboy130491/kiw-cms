                ...
                <div class="flex flex-col gap-10 lg:w-1/2">
                    ...
                    <div class="flex flex-col gap-0">
                        @if(str_contains($item->cta, 'mailto:'))
                            <h3>{{ __('career.send_resume') }}</h3>
                        @else
                            <h3>{{ __('career.apply_to') }}</h3>
                        @endif
                        <!--button-->
                        <a class="w-fit btn1 mt-5" data-aos="fade-down" href="{{ $item->cta }}"
                            target="_blank">{{ $item->cta_label ?? 'Apply' }}
                            <span>
                    ...
                    @if(str_contains($item->cta, 'mailto:'))
                        <div class="flex flex-col gap-5">
                            <h3>{{ __('career.subject_email') }}</h3>
                            <p class="text-[var(--color-blue)]">
                                {{ $item->careerCategories->first()?->title ?? 'Full Time' }} - {{ $item->title }} -
                                {{ __('career.full_name') }}
                            </p>
                        </div>
                    @endif
        ...
        <section id="karir-lainnya" class="bg-[var(--color-transit)]">
            <div class="px-4 sm:px-6 lg:px-0 lg:max-w-[1200px] lg:mx-auto lg:py-30 py-18 flex flex-col gap-8">
                <div class="flex sm:flex-row flex-col gap-5 justify-between">
                    <h2 data-aos="fade-up">{{ __('career.other_positions') }}</h2>
                    <!--button-->
                    <a class="w-fit btn1 mt-5 !hidden sm:!flex" data-aos="fade-down" href="{{$karir_archive_url}}"
                        target="_blank">{{ __('career.other_vacancies') }}
                        <span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                fill="none">
                                <path d="M5 12H19" stroke="white" stroke-width="2" stroke-linecap="round"
                ...
                <a class="w-fit btn1 mt-5 sm:!hidden !flex" data-aos="fade-down" href="{{$karir_archive_url}}"
                    target="_blank">{{ __('career.other_vacancies') }}
                    <span>
