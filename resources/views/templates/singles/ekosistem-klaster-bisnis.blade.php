...
<x-layouts.app>
    ...
    <main>
        <x-partials.hero-page :image="$item->featuredImage?->url ?? Storage::url('media/bdc43b57-5e80-418a-9a89-47cf20e59746.jpg')"
            h1="{{ $item->title ?? 'Ekosistem Klaster Bisnis' }}" />
            ...
            <div x-data="{ tab: 'tab1' }" class="rounded-md">
                ...
                <div
                    ...
                    @foreach ($tabSektorIndustri as $index => $item)
                        <x-tab.tab-headers-sektor :title="$item['tabTitle']" :tab="'tab' . ($index + 1)" />
                    @endforeach
                </div>
