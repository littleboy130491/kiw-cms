@php
    ...
    $section = $block->where('data.block_id', 'section-lelang')->first();

    $description = $section['data']['description'] ?? $record->content ?? '';
@endphp
<x-layouts.app>
    <x-partials.header />
    <main>
        ...
        <section id="post-archive"
            ...
            <div class="flex flex-col lg:flex-row lg:items-start gap-5 lg:gap-25">
                <h2 class="lg:w-1/3" data-aos="fade-up">
                    {{$section['data']['title'] ?? 'Pedoman Lelang' }}
                </h2>

                <div class="lg:w-2/3 flex flex-col gap-5">
                    <p>{!! $description !!}</p>
                    <!--button-->
                    <a class="w-fit btn1 mt-5" data-aos="fade-down" href="{{ $section['data']['url'] ?? '#' }}">
                        {{ $section['data']['button_label'] ?? 'Unduh Pedoman' }}
