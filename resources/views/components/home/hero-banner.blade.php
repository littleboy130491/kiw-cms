                        ...
                        <div class="absolute inset-0 z-0">
                            @if (isset($slide['video']))
                                ...
                                <img id="video-fallback"
                                    src="{{ $slide['fallback_image'] ?? Storage::url('media/background-home.jpg') }}"
                                ...
                                <iframe id="video-frame"
                                    ...
                                    src="{{ $slide['video'] }}" title="Background video" frameborder="0"
                                    allow="autoplay; encrypted-media" allowfullscreen></iframe>
                            @else
                                {{-- Image Background --}}
                                <x-ui.responsive-image :src="$slide['image']" :alt="$slide['alt'] ?? 'Hero background'"
                                        ...
                                        @if (isset($slide['buttons']))
                                            <div class="flex flex-wrap gap-4 mt-6">
                                                @foreach ($slide['buttons'] as $button)
                                                    <x-ui.button :href="$button['href'] ?? '#'" :variant="$button['variant'] ?? 'primary'" :size="$button['size'] ?? 'lg'"
                                                        :target="$button['target'] ?? null" class="min-w-[150px]">
                                                        {{ $button['text'] }}
                                                    </x-ui.button>
                                                @endforeach
                                            </div>
