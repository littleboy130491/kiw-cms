@props([
    ...
    'backgroundImage' => Storage::url('media/back-keunggulan.jpg'),
    'advantages' => [
        [
            'number' => '01.',
...
<section id="keunggulan-home" class="bg-no-repeat bg-cover relative"
            ...
            <x-ui.container>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    @foreach ($advantages as $index => $advantage)
                        <x-home.partials.advantage-card :number="$advantage['number']" :title="$advantage['title']" :description="$advantage['description']"
                            :url="$advantage['url']" :delay="$index * 100" />
                    @endforeach
                </div>
            </x-ui.container>
        </div>
