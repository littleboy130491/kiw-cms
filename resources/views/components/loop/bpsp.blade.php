<a href="{{ route('cms.single.content', ['lang' => app()->getLocale(), 'content_type_key' => 'bpsp', 'content_slug' => $item->slug]) }}"
    ...
    @if ($item->buildingCategories->first())
        @if ($item->buildingCategories->first()?->slug === 'tersedia' || $item->buildingCategories->first()?->slug === 'available')
            <div class="gradient-green top-0 left-0 w-fit absolute px-3 py-2 rounded-tl-md rounded-br-md blinking">
        @else
