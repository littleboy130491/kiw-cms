...
<!--Item-->
<div class="tender-loop group transition-all duration-[500ms] relative flex flex-col bg-[var(--color-transit)] rounded-md overflow-hidden">
    ...
    <div class="con-front flex flex-col justify-between grow gap-6 px-6 pt-6">
        ...
        <div>
            <img class="rounded-md rounded-b-none lg:h-[210px] sm:h-[300px] h-[250px] w-full object-cover object-top" src="{{ $item_image }}">
        </div>
        
    </div>

    <!--back-->
    <div class="con-back absolute group-hover:top-[0%] top-[100%] transition-all duration-[500ms] flex flex-col justify-end bg-[var(--color-transit)] rounded-md gap-15 px-6 pt-6 h-full w-full">
        <div
            class="gradient-blue top-0 left-0 w-fit absolute px-3 py-2 rounded-tl-md rounded-br-md {{ $item->tenderStatus->first()?->slug === 'terbaru' ? 'blinking' : '' }}">
            <p class="text-white uppercase text-[.8em]">{{ $item->tenderStatus->first()?->title ?? 'terbaru' }}</p>
        </div>
