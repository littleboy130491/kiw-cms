@php
    ...
    $textSpinner = '<svg class="animate-spin h-4 w-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>';
@endphp
                        ...
                        <div class="flex flex-row justify-between items-center mt-5">
                            <div class="flex gap-2">
                                ...
                                @if($comment->all_children_count > 0)
                                    <button wire:click="toggleReplies({{ $comment->id }})" wire:loading.attr="disabled" wire:loading.class="cursor-wait" wire:target="toggleReplies({{ $comment->id }})" class="cursor-pointer flex items-center text-gray-500 hover:text-gray-700 text-[.85em]">
                                    <span wire:loading wire:target="toggleReplies({{ $comment->id }})" class="mr-2">
                                        {!! $textSpinner !!}
                                    </span>
